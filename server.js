...
import { Authorize, DATABASE, InvalidDb, SETTINGS_COLLECTION, get_collection, set_DATABASE } from "./GLOBAL.js";
...
server.post('/verify', (req, res) => {  // Verifying credentials for App
    ...
    switch (req.body?.password) {
        ...
        case process.env.MASTER_PASSWORD:
            type = "MASTER"; break;
        case process.env.HANDLER_PASSWORD:
            type = "HANDLER"; break;
        case process.env.VIEWER_PASSWORD:
            type = "VIEWER"; break;
        case process.env.TESTER_PASSWORD:
            type = "TESTER"; break;
    }

    let is_valid = Boolean(type)
...
server.get("/settings", (req, res) => {  // Get settings from database
    ...
    Authorize(req, [process.env.DEV_PASSWORD, process.env.MASTER_PASSWORD, process.env.TESTER_PASSWORD])

    DATABASE.collection(get_collection(SETTINGS_COLLECTION, req)).findOne({})
        .then((result, err) => {
            res.json({
                is_valid: true,
                data: result
...
server.post("/settings", (req, res) => {  // Get settings from database
    ...
    DATABASE.collection(get_collection(SETTINGS_COLLECTION, req)).updateOne({ _id: new ObjectId(_id) }, { $set: req.body },{ upsert: true })
        .then(result => {
